...
class Tester:
    def __init__(self, model_path: str, img_channels: int = 3, use_gpu: bool = True):
    ...
    def evaluate(self, dataset: str, dataroot: str, mask_root: str, batch_size: int):
        ...
        mse, psnr, ssim = 0, 0, 0
        total = 0
        for X, img, noise, mask in tqdm(test_loader, desc='Evaluating', ncols=120, leave=False, colour='yellow'):
            bs = X.shape[0]
            img = img.to(device=self.device, dtype=torch.float32)
            X = X.to(device=self.device, dtype=torch.float32)
            mask = mask.expand(X.shape).to(device=self.device, dtype=torch.float32)
            inpainted_img = self.G(X, mask)
            mse += metrics.MSE((img + 1) / 2, (inpainted_img + 1) / 2, batch=True) * bs
            psnr += metrics.PSNR((img + 1) / 2, (inpainted_img + 1) / 2, batch=True) * bs
            ssim += metrics.SSIM((img + 1) / 2, (inpainted_img + 1) / 2, batch=True) * bs
            total += bs
        return mse / total, psnr / total, ssim / total

    @torch.no_grad()
    def predict_single(self, img_path, mask_path, save_path):
        assert os.path.exists(img_path) and os.path.isfile(mask_path) and os.path.isfile(img_path)
        self.G.eval()
        transforms = T.Compose([T.Resize((128, 128)), T.ToTensor(), T.Normalize(mean=[0.5]*self.img_channels, std=[0.5]*self.img_channels)])
        mask_transforms = T.Compose([T.Resize((128, 128)), T.ToTensor()])
        img = Image.open(img_path)
        img = transforms(img).unsqueeze(0)
        img = img.to(device=self.device)
        mask = Image.open(mask_path)
        mask = mask_transforms(mask).unsqueeze(0)
        mask = mask.expand(img.shape).to(device=self.device)
        img = self.G(img, mask)
    ...
    def predict(self, predict_dir):
        ...
        mask_dir = os.path.join(predict_dir, 'mask')
        ...
        for file in files:
            ...
            if os.path.isfile(img_path) and os.path.splitext(file)[1] in ['.jpg', '.png', '.jpeg']:
                ...
                self.predict_single(img_path=img_path, mask_path=mask_path, save_path=save_path)
